...
@AutoConfiguration
@ComponentScan(basePackages = "com.pizzeria.pizza", nameGenerator = PizzaBeanNameGenerator.class)
public class PizzaAutoConfiguration {

    /** Le four a pizza. Nom du bean = nom de la methode = "four". */
    @Bean
    public Four fourAPizza() {
        return new Four("pizza", 450);
    }

    @Bean
    public Module pizzaModule(Four fourAPizza, Caisse caisse) {
        return new Module() {
            public String nom()   { return "pizza"; }
            public String carte() { return "Margherita cuite a " + fourAPizza.temperature() + "°C"
